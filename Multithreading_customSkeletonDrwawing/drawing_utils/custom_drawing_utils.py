...
_PRESENCE_THRESHOLD = 0.1  # default 0.5
_VISIBILITY_THRESHOLD = 0.5  # default 0.5
_BGR_CHANNELS = 3

WHITE_COLOR = (224, 224, 224)
BLACK_COLOR = (14, 14, 14)  # New shade of black color
RED_COLOR = (0, 0, 255)
GREEN_COLOR = (0, 128, 0)
BLUE_COLOR = (255, 0, 0)
...
def draw_landmarks(
    ...
    if connections:
        ...
        for connection in connections:
            ...
            if start_idx in idx_to_coordinates and end_idx in idx_to_coordinates:
                ...
                # Create a unit vector in the direction of the line
                line_vector = np.array(
                    idx_to_coordinates[start_idx]) - np.array(idx_to_coordinates[end_idx])
                unit_vector = line_vector / np.linalg.norm(line_vector)

                # Calculate the new start and end points to shorten the line by 2 pixels
                shorten_distance = - 17
                new_start_point = np.array(
                    idx_to_coordinates[start_idx]) + (unit_vector * shorten_distance)
                new_end_point = np.array(
                    idx_to_coordinates[end_idx]) - (unit_vector * shorten_distance)

                # Draw the colored line on the original image
                cv2.line(image, tuple(new_start_point.astype(int)), tuple(new_end_point.astype(
                    int)), BLACK_COLOR, drawing_spec.thickness + 2, lineType=cv2.LINE_AA)

                cv2.line(image, tuple(new_start_point.astype(int)),
                         tuple(new_end_point.astype(int)), drawing_spec.color,
                         drawing_spec.thickness, lineType=cv2.LINE_AA)
    ...
    if landmark_drawing_spec:
        for idx, landmark_px in idx_to_coordinates.items():
            ...
            if drawing_spec.thickness is None:
                continue
            # White circle border
            ...
            cv2.circle(image, landmark_px, circle_border_radius, BLACK_COLOR,
                       drawing_spec.thickness + 1, lineType=cv2.LINE_AA)
            ...
            cv2.circle(image, landmark_px, drawing_spec.circle_radius,
                       drawing_spec.color, drawing_spec.thickness, lineType=cv2.LINE_AA)
